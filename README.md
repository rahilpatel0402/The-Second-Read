...
**Case 3 — Harold Byrne, 76 M, SNF day 9, new-onset atrial fibrillation ("the stale record").** The note is drafted at 10:05 during morning rounds — apixaban correctly started, cardiology concurring in writing. Like most notes, it sits in the signature queue until the afternoon. By the time the physician signs, the 14:20 nursing MAR reconciliation shows anticoagulation still on hold, no apixaban on the active MAR, and no new orders received: the order never landed. → **The Second Read flags the record, not the doctor** — the query routes to nursing/pharmacy to reconcile the MAR before the first dose is missed. This case proves two things: the tool is bidirectional (it vindicates the provider when the chart is stale), and the draft-to-signature gap is precisely where a note quietly stops describing the patient.
